# Agent Guide — Cursor Orchestrator

Instructions for AI coding agents and contributors working in this repository.

## What this project is

**Cursor Orchestrator** is a Windows-first framework for running multi-phase agent workflows via the Cursor SDK. Workflows are YAML/JSON files that define agents, phases, dependencies, and acceptance criteria. Each run persists state and artifacts under `.runs/<run-id>/`.
...
Live agent runs require `CURSOR_API_KEY`. Use `--dry-run` or `MockAgentRunner` when you should not call the API.
...
- Importing `@cursor/sdk` outside `src/runners/`.
...
- Use `MockAgentRunner` for tests that should not call Cursor APIs.
