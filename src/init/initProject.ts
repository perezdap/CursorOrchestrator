...
Local configuration for [Cursor Orchestrator](https://github.com/perezdap/CursorOrchestrator).
...
| Setting | Description |
|---------|-------------|
| \`defaultExecutionMode\` | \`local\` or \`cloud\` for Cursor agent runs |
...
export function formatInitSummary(result: InitProjectResult): string {
  ...
  lines.push("");
  lines.push("Optional dry run (no Cursor API calls):");
